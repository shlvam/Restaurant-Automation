...
<?php
	if(isset($_POST['tab_id'])){
		...
		$sql_qu="UPDATE `restaurant`.`tab` SET `tab_id`='$tab_id',`nchair`='$n_chair' WHERE `tab_id`='$tab_id'";
		if($con->query($sql_qu) == true){
			?>
			<META HTTP-EQUIV="Refresh" CONTENT="0; URL=http://localhost/restaurant/tab.php">
			<?php
...
<html lang="en">
  <head>
    ...
    <title>Information Update - Table</title>
  </head>
  <body>
...
<!-- main code -->
	<form action="tabedit.php" method="post">
		<tr>
			<td>Table Id</td>
            <td><input type="int" name="tab_id" id="tab_id" value="<?php echo "$ti" ?>"></td>
        </tr>
        <tr>   
        	<td>No of chairs</td> 
            <td><input type="int" name="n_chair" id="n_chair" value="<?php echo "$nc" ?>"></td>
        </tr>
        <tr>    
