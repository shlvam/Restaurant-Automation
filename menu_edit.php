...
<?php
	if(isset($_GET['ti'])){
	    ...
	    $sql_show="SELECT * FROM `restaurant`.`menu` WHERE `menuid`='$menuid'";
...
<?php
    ...
    $price = $_POST['price'];
		...
		$sql_qu="UPDATE `restaurant`.`menu` SET `menuid`='$menuid',`food_name`='$food_name',`food_pic`='$food_pic', `price`='$price' WHERE `menuid`='$menuid'";
		if($con->query($sql_qu) == true){
			?>
			<META HTTP-EQUIV="Refresh" CONTENT="0; URL=http://localhost/restaurant/menu.php">
			<?php
		}else{
			echo"<script>
...
<html lang="en">
  <head>
    ...
    <title>Information Update - Menu</title>
  </head>
  <body>
...
<!-- main code -->
    <form action="menu_edit.php" method="post">
        <tr>
            <td>Menu Id</td>
            <td><input type="int" name="menuid" id="menuid" value="<?php echo "$row[menuid]" ?>"></td>
        </tr>
        <tr>   
            <td>Food Name</td> 
            <td><input type="text" name="food_name" id="food_name" value="<?php echo "$row[food_name]" ?>"></td>
        </tr>
        <tr>   
            <td>Food Image</td> 
            <td><input type="text" name="food_pic" id="food_pic" value="<?php echo "$row[food_pic]" ?>"></td>
        </tr>        
        <tr>   
            <td>Price</td> 
            <td><input type="number" step="any" name="price" value="<?php echo "$row[price]" ?>"></td>
        </tr>
        <tr>    
